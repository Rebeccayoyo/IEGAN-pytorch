...
from infrared import infraredDatah5test
from cv2 import cv2
import math
...
class ImageContraster():
    ...
    def plot_images(self, img1, img2,method):
        ### draw two images
...
if __name__ == '__main__':
    ...
    out_dir = '/home/ipsg/code/ZLJ/Project/Infrared/torch_lua_IEGAN/IE-GAN_Pytorch_zlj/直方图均衡结果/'
    psnr_HE,psnr_AHE,psnr_CLAHE,psnr_Bright= 0,0,0,0
    icter = ImageContraster()
    for image_id,(batch_x,batch_y) in enumerate(testset):
        batch_x = batch_x.permute(1,2,0)
        batch_y = batch_y.permute(1,2,0)
        img = batch_x.detach().numpy()*255
        batch_y = batch_y.detach().numpy()*255        
        
        img = img.astype(np.int32)
        # HE
        he_eq_img = icter.enhance_contrast(img, method = "HE")
        psnr_HE += cal_psnr(he_eq_img,batch_y,255)
        all = np.concatenate([img,he_eq_img,batch_y],axis=1)
        cv2.imwrite(out_dir+str(image_id)+'HE.bmp',all)
        
        # AHE
        # ahe_eq_img = icter.enhance_contrast(img, method = "AHE", window_size = 32, affect_size = 16)
        # psnr_AHE += cal_psnr(ahe_eq_img,batch_y,255)
        # all = np.concatenate([img,ahe_eq_img,batch_y],axis=1)
        # cv2.imwrite(out_dir+str(image_id)+'AHE.bmp',all)

        # CLAHE
        clahe_eq_img = icter.enhance_contrast(img, method = "CLAHE", blocks = 8, threshold = 10.0)
        psnr_CLAHE += cal_psnr(clahe_eq_img,batch_y,255)
        all = np.concatenate([img,clahe_eq_img,batch_y],axis=1)
        cv2.imwrite(out_dir+str(image_id)+'CLAHE.bmp',all)

        # Local Region Stretch
        lrs_eq_img = icter.enhance_contrast(img, method = "Bright")
        psnr_Bright += cal_psnr(lrs_eq_img,batch_y,255)
        all = np.concatenate([img,lrs_eq_img,batch_y],axis=1)
        cv2.imwrite(out_dir+str(image_id)+'Bright.bmp',all)
    print(psnr_HE/len(testset),psnr_AHE/len(testset),psnr_CLAHE/len(testset),psnr_Bright/len(testset))